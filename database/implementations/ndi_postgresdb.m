classdef  ndi_postgresdb < ndi_database
	...
	methods
		...
		function ndi_postgresdb_obj = ndi_postgresdb(varargin)
		% NDI_POSTGRESDB make a new NDI_POSTGRESDB object
		% 
		% NDI_POSTGRESDB_OBJ = NDI_POSTGRESDB(PATH, SESSION_UNIQUE_REFERENCE, COMMAND, ...)
		%
		% Creates a new NDI_POSTGRESDB object.
		%
    ...
    %             ndi_postgresdb_obj = ndi_postgresdb_obj@ndi_database(varargin{:});
    %             disp('pgobj:')
	...
	methods, % public
        ...
        function sqlquery = ndiquery_to_sql(ndi_postgresdb_obj, ndiquery)
            ...
            % WHERE data ->> 'list' LIKE ('%abc%')  
                ...
                assert(isa(ndiquery, 'struct'), "ndi_query not a struct")
                assert(isfield(ndiquery, 'operation'), "ndi_query object has no operation")
                assert(isfield(ndiquery, 'param1'), "ndi_query object has no param1")
                assert(isfield(ndiquery, 'param2'), "ndi_query object has no param2")
                ...
                switch ndiquery.operation
                   ...
                   case 'isa' 
                   % is 'param1' either a superclass or the document class itself of the NDI_DOCUMENT? 
                   % NDI_DOCUMENT is the field?
                   % TODO     
...
%             sqlwrite(conn,tablename,data)            
		...
		function [data, versions] = do_search(ndi_postgresdb_obj, searchoptions, searchparams)
            ...
            assert( isa(searchparams,'ndi_query'), "search params are not valid")
