| Function Name | Line Number | Line of Code | Comment |
| :--- | :--- | :--- | :--- |
| `ndi.daq.reader.mfdaq.ingest_epochfiles` | 784 | `S1 = 1+ (t0t1{1}(end) - t0t1{1}(1)) * unique(sample_rates_here_unique);` | Calculates total samples from duration and rate, assuming constant rate. |
| `ndi.daq.reader.mfdaq.ingest_epochfiles` | 811 | `S1 = 1+(t0t1{1}(end) - t0t1{1}(1)) * unique(sample_rates_here_unique);` | Calculates total samples from duration and rate, assuming constant rate. |
| `ndi.daq.reader.mfdaq.readchannels_epochsamples_ingested` | 200 | `absolute_beginning = ndi.time.fun.times2samples(t0_t1{1}(1),t0_t1{1},sr);` | Calls `ndi.time.fun.times2samples`, which assumes constant sampling rate. |
| `ndi.daq.reader.mfdaq.readchannels_epochsamples_ingested` | 201 | `absolute_end = ndi.time.fun.times2samples(t0_t1{1}(2),t0_t1{1},sr);` | Calls `ndi.time.fun.times2samples`, which assumes constant sampling rate. |
| `ndi.daq.reader.mfdaq.readevents_epochsamples` | 350 | `s0d = 1+round(srd*t0);` | Calculates sample index linearly from time, assuming constant rate. |
...
| `ndi.time.timeseries.times2samples` | 67 | `samples = 1 + round ((times-et.t0_t1{1}(1))*sr);` | Converts time to sample index using a linear formula, assuming constant rate. |
