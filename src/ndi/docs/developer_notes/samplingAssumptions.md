...
| `ndi.daq.reader.mfdaq.readevents_epochsamples_ingested` | 433 | `s1d = 1+round(srd*t1);` | Calculates sample index linearly from time, assuming constant rate. | Yes | No |
...
| `ndi.probe.timeseries.mfdaq.readtimeseriesepoch` | 81 | `s0 = 1+round(sr*t0);` | Calculates sample index from time using a constant sampling rate. | Yes | No |
| `ndi.probe.timeseries.mfdaq.readtimeseriesepoch` | 82 | `s1 = 1+round(sr*t1);` | Calculates sample index from time using a constant sampling rate. | Yes | No |
