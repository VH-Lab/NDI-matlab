...
Adding a new level (deeper coarsening after the fact) creates a new
`lightsheetZarrLevel` document, and the parent `lightsheetZarrPyramid`
records the new `n_levels` on its own next version.
...
- `level` - 0-based level index within the parent pyramid. 0 is the
  finest.
- `reduction` - the reduction of this level relative to level 0
  (`mean`, `max`). Redundant with the parent's field but stored here
  so a level can be interpreted without loading the parent.
...
- `lightsheetZarrPyramid_id` - the parent pyramid (this level's
  reduction and coordinate frame).
- `subject_id` - subject imaged.
- `source_file_id` - `fileReference` for the OME-Zarr store the level
  was materialized from. Two levels of two reductions of the same
  volume share this id.
