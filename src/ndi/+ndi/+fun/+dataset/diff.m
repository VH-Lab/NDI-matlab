...
%                           for common documents.
    ...
    for i=1:numel(common_ids)
        ...
        for f=1:numel(common_files)
            [file1_path, ~] = D1.database_get_filename_from_binarydoc(common_files{f});
            [file2_path, ~] = D2.database_get_filename_from_binarydoc(common_files{f});

            [are_identical, diff_output] = ndi.util.getHexDiff(file1_path, file2_path);
