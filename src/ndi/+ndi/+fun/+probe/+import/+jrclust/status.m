...
%   curated      - true if the sort carries curation notes ('jrc manual')
...
%
    ...
    if isfile(P.resFile),
        ...
        s.detected = ismember('spikeTimes', present);
        s.sorted   = s.detected && ismember('spikeClusters', present);
        s.curated  = s.sorted && ismember('clusterNotes', present);
    end;
