...
%             ndi.cloud.logout
    ...
    methods (Static, Access = private)
        ...
        function bytes = randomBytes(n)
            ...
            % repeat.
            %
            % A real Java primitive byte[] is allocated via reflection so that
            % nextBytes fills it in place (MATLAB passes plain arrays to Java by
            % value, which would drop the generated bytes); it is then read back
            % into an int8 row vector to preserve this method's contract.
