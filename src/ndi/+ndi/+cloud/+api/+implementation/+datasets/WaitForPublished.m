classdef WaitForPublished < ndi.cloud.api.call
%WAITFORPUBLISHED Polls a dataset until its 'isPublished' flag becomes true.
%
%   Implementation class behind ndi.cloud.api.datasets.waitForPublished.
%   Polls GET /datasets/{cloudDatasetID} at exponentially growing intervals
%   (capped) until answer.isPublished is true or the overall timeout
%   elapses.
    ...
    methods
        function this = WaitForPublished(args)
            %WAITFORPUBLISHED Creates a new WaitForPublished call.
            arguments
        ...
        function [b, answer, apiResponse, apiURL] = execute(this)
            %EXECUTE Poll getDataset until isPublished is true or timeout.
            %
            %   b           - true iff the dataset reached isPublished=true
            %                 before the timeout. false on API error or
            %                 timeout.
            ...
            while true
                ...
                if ok && isstruct(ds) && isfield(ds, 'isPublished') && ...
                        ~isempty(ds.isPublished) && logical(ds.isPublished)
                    b = true;
                    return;
                end
