...
methods (Access = private, Static)
    ...
    function strain = createStrainObject(tableRow, species)
        ...
        try
            ...
            strainName = tableRow.StrainName{1};
            if strcmp(strainName, 'N2')
                ...
                try
                    [id,name,~,definition,synonyms] = ndi.ontology.lookup(['WBStrain:',strainName]);
                catch
                    [id,name,~,definition,synonyms] = ndi.ontology.lookup(['EMPTY:',strainName]);
                end
                st_trans.name = name;
    ...
    function subjectIdentifier = constructSubjectIdentifier(tableRow)
        ...
        try
            ...
            subjectIdentifier = [baseString, '@babu-lab.iisc.ac.in'];
        catch ME
