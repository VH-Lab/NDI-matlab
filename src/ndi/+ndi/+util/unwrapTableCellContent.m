...
%
    ...
    % If the final unwrapped value is an empty array (like []), also treat it as NaN,
    % but be sure to preserve empty char arrays ('') which are valid.
    if isempty(unwrappedValue) && ~ischar(unwrappedValue)
        unwrappedValue = NaN;
    end
