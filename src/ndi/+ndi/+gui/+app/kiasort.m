...
%             ndi.fun.probe.import.kiasort.run, ndi.fun.probe.import.kiasort.status
    ...
    properties (Access = private)
        ...
        ConfigNums              % containers.Map: cfg field -> numeric uieditfield handle
    ...
    methods (Access = private)
        function build(obj)
            ...
            obj.fig = uifigure('Name', ['KIASORT: ' obj.session.reference], ...
                'Position', [100 100 560 545], ...
                'Color', c.darkBlue, ...
                'Tag', 'ndi.gui.app.kiasort');

            root = uigridlayout(obj.fig, [7 1], ...
                'RowHeight', {30, 18, '1x', 18, 100, 34, 44}, 'ColumnWidth', {'1x'}, ...
            ...
            % Numeric KIASORT parameters (field, label, default, min, integer, tooltip),
            % laid out as [label field label field ...]. Passed to run as cfg_overrides.
            numDefs = { ...
                'sortingChunkDuration', 'Sort chunk (s)', 120, 1, false, ...
                    ['Duration (s) of each stage-3 sorting chunk (the "Chunk k/N" loop). ' ...
                     'Larger = fewer passes over the recording (faster) but more RAM per ' ...
                     'chunk (~num_channels x duration x fs x 8 bytes). Default 120.']; ...
                'batch_ch_size', 'Chan. batch', 64, 1, true, ...
                    ['Channels loaded and processed per batch within a chunk. Larger reduces ' ...
                     'redundant reads over the same time span at the cost of more RAM. Set it to ' ...
                     'your channel count - or just a high number, KIASORT caps it at the number ' ...
                     'of channels - to load all channels at once (one read per chunk). Default 64.']};

            numRow = uigridlayout(root, [1 5], ...
                'ColumnWidth', {110, 70, 90, 70, '1x'}, 'RowHeight', {'1x'}, ...
                'ColumnSpacing', 6, 'Padding', [0 0 0 0], ...
                'BackgroundColor', c.darkBlue);
            numRow.Layout.Row = 6; numRow.Layout.Column = 1;

            obj.ConfigNums = containers.Map('KeyType', 'char', 'ValueType', 'any');
            for i = 1:size(numDefs,1)
                lbl = uilabel(numRow, 'Text', [numDefs{i,2} ':'], ...
                    'FontColor', c.white, 'HorizontalAlignment', 'right', ...
                    'VerticalAlignment', 'center');
                lbl.Layout.Column = 2*i - 1;
                ef = uieditfield(numRow, 'numeric', 'Value', numDefs{i,3}, ...
                    'Limits', [numDefs{i,4} Inf], 'LowerLimitInclusive', 'on', ...
                    'RoundFractionalValues', onOff(numDefs{i,5}), ...
                    'BackgroundColor', c.white, 'FontColor', c.darkBlue, ...
                    'Tooltip', numDefs{i,6});
                ef.Layout.Column = 2*i;
                obj.ConfigNums(numDefs{i,1}) = ef;
            end
            ...
            bottom = uigridlayout(root, [1 4], ...
                ...
                'BackgroundColor', c.darkBlue);
            bottom.Layout.Row = 7; bottom.Layout.Column = 1;
        ...
        function cfg = configOverrides(obj)
            % Build the cfg_overrides struct from the checkbox and numeric-field states.
            cfg = struct();
            keys = obj.ConfigChecks.keys;
            for i = 1:numel(keys)
                cfg.(keys{i}) = logical(obj.ConfigChecks(keys{i}).Value);
            end
            if ~isempty(obj.ConfigNums)
                nkeys = obj.ConfigNums.keys;
                for i = 1:numel(nkeys)
                    cfg.(nkeys{i}) = obj.ConfigNums(nkeys{i}).Value;
                end
            end
        end
