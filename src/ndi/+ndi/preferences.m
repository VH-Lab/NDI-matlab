...
%   See also: ndi.gui.preferencesEditor, jsonencode, jsondecode
    ...
    methods (Access = private)
        ...
        %   preferences editor.
            ...
            obj.addItem('GUI', 'Navigator', 'SessionAppPackages', ...
                "", 'string', ...
                ['Extra package names (semicolon- or comma-separated) to scan ' ...
                 'for session GUI apps in the navigator Apps menu, e.g. ' ...
            ...
            obj.addItem('GUI', 'GEFManager', 'ViewerLauncher', ...
                "/usr/local/bin/napariViewGEF", 'string', ...
                ['Program the GEF Manager''s View button runs to open a ' ...
                 'pyramid in napari. Normally a small shell wrapper rather ' ...
                 'than the napariViewGEF console script itself: MATLAB ' ...
                 'exports library paths of its own, and a Python process ' ...
                 'started from MATLAB loads MATLAB''s copies of libraries ' ...
                 'it must not use, so the wrapper scrubs the environment ' ...
                 'before running the real entry point.']);
            obj.addItem('Parallel', "", 'OpenPoolIfNone', ...
                false, 'logical', ...
                ['Open a parallel pool when a calculation reaches a parallel ' ...
                 'loop and no pool is running. When false, NDI never starts a ' ...
                 'pool of its own: a calculation uses a pool you have already ' ...
                 'opened, and otherwise runs serially. Each worker is a ' ...
                 'separate MATLAB process, so a pool nobody asked for costs ' ...
                 'both start-up time and a multiple of MATLAB''s memory.']);
            obj.addItem('Parallel', "", 'UseDefaultNumberOfWorkers', ...
                true, 'logical', ...
                ['Let MATLAB decide how many workers a parallel loop may use: ' ...
                 'a pool you have already opened is used at its own size, and ' ...
                 'a pool opened by NDI takes the MATLAB default, normally one ' ...
                 'worker per core. Turn this off to set the number yourself in ' ...
                 'NumberOfWorkers, which is worth doing on a machine with ' ...
                 'limited memory: each worker is a separate MATLAB process, so ' ...
                 'one per core can be more than the machine can hold.']);
            obj.addItem('Parallel', "", 'NumberOfWorkers', ...
                2, 'double', ...
                ['How many workers a parallel loop may use, when ' ...
                 'UseDefaultNumberOfWorkers is off. Ignored while that is on. ' ...
                 'This is an upper limit: a pool smaller than this is used at ' ...
                 'its own size, not enlarged.']);
        end
