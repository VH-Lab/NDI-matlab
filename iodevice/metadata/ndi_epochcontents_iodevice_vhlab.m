classdef ndi_epochcontents_iodevice_vhlab < ndi_epochcontents_iodevice
	properties
	end % properties
	methods
		function obj = ndi_epochcontents_iodevice_vhlab(name_, reference_, type_, devicestring_)
			% NDI_EPOCHCONTENTS_IODEVICE_VHLAB - Create a new ndi_epochcontents_iodevice object derived from the vhlab device implementation
			%
			% MYNDI_EPOCHCONTENTS_IODEVICE = NDI_EPOCHCONTENTS_VHLAB(NAME, REFERENCE, TYPE, DEVICESTRING)
			%
			...
			% this record.
			% 
			%   MYNDI_EPOCHCONTENTS_IODEVICE = NDI_EPOCHCONTENTS_VHLAB(FILENAME)
			% 
			...
			obj = obj@ndi_epochcontents_iodevice(name_, reference_, type_, devicestring_);
			...
			if nargin==1,
				...
				if strcmp([localfile ext],'stimtimes.txt'), % vhvis_spike2
					...
					for i=1:numel(mylist),
						nextentry = ndi_epochcontents_iodevice_vhlab('vhvis_spike2',...
							1,...
							['stimulator'  ] , ...  % type
		...
		% 
		%  Writes the NDI_EPOCHCONTENTS_IODEVICE_VHLAB object to disk in filename FILENAME (full path).
		%
		%  
