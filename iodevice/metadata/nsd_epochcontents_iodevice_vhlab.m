classdef nsd_epochcontents_iodevice_vhlab < nsd_epochcontents_iodevice
	...
	methods
		function obj = nsd_epochcontents_iodevice_vhlab(name_, reference_, type_, devicestring_)
			% NSD_EPOCHCONTENTS_IODEVICE_VHLAB - Create a new nsd_epochcontents_iodevice object derived from the vhlab device implementation
			%
			% MYNSD_EPOCHCONTENTS_IODEVICE = NSD_EPOCHCONTENTS_VHLAB(NAME, REFERENCE, TYPE, DEVICESTRING)
			%
			...
			% this record.
			% 
			%   MYNSD_EPOCHCONTENTS_IODEVICE = NSD_EPOCHCONTENTS_VHLAB(FILENAME)
			% 
			...
			obj = obj@nsd_epochcontents_iodevice(name_, reference_, type_, devicestring_);
			...
			if nargin==1,
				...
				if strcmp([localfile ext],'stimtimes.txt'), % vhvis_spike2
					...
					for i=1:numel(mylist),
						nextentry = nsd_epochcontents_iodevice_vhlab('vhvis_spike2',...
							1,...
							['stimulator'  ] , ...  % type
				...
				for i=1:length(nsd_struct),
					...
					nextentry = nsd_epochcontents_iodevice_vhlab(nsd_struct(i).name,...
							nsd_struct(i).ref,...
							ec_type, ...  % type
		...
		% 
		%  Writes the NSD_EPOCHCONTENTS_IODEVICE_VHLAB object to disk in filename FILENAME (full path).
		%
		%  
