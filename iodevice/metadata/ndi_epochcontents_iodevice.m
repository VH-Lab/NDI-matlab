classdef ndi_epochcontents_iodevice < ndi_epochcontents
	properties
	...
	methods
		function obj = ndi_epochcontents_iodevice(name_, reference_, type_, devicestring_)
			% NDI_EPOCHCONTENTS_IODEVICE - Create a new ndi_epochcontents_iodevice object
			%
			% MYNDI_EPOCHCONTENTS_IODEVICE = NDI_EPOCHCONTENTS(NAME, REFERENCE, TYPE, DEVICESTRING)
			%
			% Creates a new NDI_EPOCHCONTENTS_IODEVICE with name NAME, reference REFERENCE, type TYPE,
			...
			if nargin==1,
				...
				for i=1:length(ndi_struct),
					nextentry = ndi_epochcontents_iodevice(ndi_struct(i).name,...
				...
				if isempty(obj),
					obj = ndi_epochcontents_iodevice;
					obj = obj([]);
				end
				return;
			end;
			...
			if reference_ < 0 | ~isint(reference_) | ~eqlen(size(reference_),[1 1]),
				error(['reference of ndi_epochcontents_iodevice must be a non-negative scalar integer, got ' int2str(reference_)]);
			end;
		...
		function savetofile(ndi_epochcontents_iodevice_obj, filename)
		%  SAVETOFILE - Write ndi_epochcontents_iodevice object array to disk
		%
                %  SAVETOFILE(NDI_EPOCHCONTENTS_IODEVICE_OBJ, FILENAME)
		%
		%  Writes the NDI_EPOCHCONTENTS_IODEVICE object to disk in filename FILENAME (full path).
		%
		%
