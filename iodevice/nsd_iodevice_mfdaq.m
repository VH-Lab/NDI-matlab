% NSD_IODEVICE_MFDAQ - Multifunction DAQ object class
%
% The NSD_IODEVICE_MFDAQ object class.
%
...
classdef nsd_iodevice_mfdaq < nsd_iodevice
	...
	methods
		function obj = nsd_iodevice_mfdaq(varargin)
			% NSD_IODEVICE_MFDAQ - Create a new multifunction DAQ object
			%
			%  D = NSD_IODEVICE_MFDAQ(NAME, THEFILETREE)
			%
		...
		function [t_prime, epochnumber_prime] = timeconvert_old(self, clock, t, epochnumber)
			...
			%
				...
				if ~ismyclock, % need to send out to sync table for conversion
					exp = self.experiment;
					myclock = nsd_clock_iodevice('dev_local_time',self); % MORE HERE
					if nargin<4,
				...
				if isa(clock,'nsd_clock_iodevice_epoch'), % don't need epochnumber, we know it already
					t_prime = t;
	...
	methods (Static), % functions that don't need the object
		function ct = mfdaq_channeltypes
			% MFDAQ_CHANNELTYPES - channel types for NSD_IODEVICE_MFDAQ objects
			%
			%  CT = MFDAQ_CHANNELTYPES - channel types for NSD_IODEVICE_MFDAQ objects
			%
			%  Returns a cell array of strings of supported channels of the
			%  NSD_IODEVICE_MFDAQ class. These are the following:
			%
			...
			%  marker              | 
			%
			% See also: NSD_IODEVICE_MFDAQ/MFDAQ_TYPE
		...
		function type = mfdaq_type(channeltype)
			...
			% 'event' or 'e'              | 'event'
			%
			% See also: NSD_IODEVICE_MFDAQ/MFDAQ_PREFIX
			%
				switch channeltype,
					case {'analog_in','ai'},
						type = 'analog_in';
