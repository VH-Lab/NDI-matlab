classdef nsd_variable_dir < nsd_dbleaf_branch
	% NSD_VARIABLE_DIR - A variable directory class for NSD
	%
	% NSD_VARIABLE_DIR
	%
	%
	...
	methods
		function obj = nsd_variable_dir(parent, name, classnames, isflat)

			% NSD_VARIABLE_DIR - Create an NSD_VARIABLE_DIR object
			%
			%  OBJ = NSD_VARIABLE_DIR(PARENT, NAME, CLASSNAMES, [ISFLAT])
			%
			...
			% indicates that the NSD_VARIABLE_DIR should be 'flat' and not allow additional
			% subdirectories.
			%
			...
			if ~isempty(parent)&~isa(parent,'nsd_dbleaf_branch'),
				error(['NSD_VARIABLE_DIR objects must be attached to NSD_DBLEAF_BRANCH objects.']);
			end;
