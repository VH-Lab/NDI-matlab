	...
	methods
		...
		function doc = add_appdoc(ndi_appdoc_obj, appdoc_type, appdoc_struct, docexistsaction, varargin)
			...
			%
				...
				doc = ndi_appdoc_obj.find_appdoc(appdoc_type, varargin{:});
				...
				if ~isempty(doc),
					switch (lower(docexistsaction)),
						...
						case {'replace','replaceifdifferent'},
							...
							if aredifferent,
								b = ndi_appdoc_obj.clear_appdoc(appdoc_type, varargin{:});
								if ~b,
				...
				doc = ndi_appdoc_obj.struct2doc(appdoc_type,appdoc_struct,varargin{:});

				ndi_appdoc_obj.doc_session.database_add(doc);
		...
		function appdoc_struct = defaultstruct_appdoc(ndi_appdoc_obj, appdoc_type)
		...
		function varargout = loaddata_appdoc(ndi_appdoc_obj, appdoc_type, varargin)
		...
		function b = clear_appdoc(ndi_appdoc_obj, appdoc_type, varargin)
			...
			%
				...
				doc = ndi_appdoc_obj.find_appdoc(appdoc_type,varargin{:});
				if ~isempty(doc),
					ndi_appdoc_obj.doc_session.database_rm(doc);
					b = 1;
				end;
