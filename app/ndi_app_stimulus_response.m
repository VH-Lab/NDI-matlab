classdef ndi_app_stimulus_response < ndi_app
	...
	methods
		...
		function doc = compute_stimulus_response_summary(ndi_app_stimulus_response_obj, ndi_probe_stim, ndi_probe_measure, timeref, t0, t1, varargin)
			% COMPUTE_STIMULUS_RESPONSE_SUMMARY - compute responses to a stimulus set
			%
			% DOC = COMPUTE_STIMULUS_RESPONSE_SUMMARY(NDI_APP_STIMULUS_RESPONSE_APP, NDI_PROBE_STIM, NDI_PROBE_MEASURE, TIMEREF, T0, T1, ...)
			%
			% 
			%
		...
		function b = savevalidinterval(ndi_app_stimulus_response_obj, ndi_epochset_obj, validintervalstruct)
			% SAVEVALIDINTERVAL - save a valid interval structure to the experiment database
			%
			% B = SAVEVALIDINTERVAL(NDI_APP_STIMULUS_RESPONSE_OBJ, NDI_EPOCHSET_OBJ, VALIDINTERVALSTRUCT)
			%
			...
			%
				...
				[vi,mydoc] = ndi_app_stimulus_response_obj.loadvalidinterval(ndi_epochset_obj);
				b = 1;
				...
				ndi_app_stimulus_response_obj.clearvalidinterval(ndi_epochset_obj);
				newdoc = ndi_app_stimulus_response_obj.experiment.newdocument('apps/markgarbage/valid_interval',...
						'valid_interval',vi) +  ...
					ndi_epochset_obj.newdocument() + ndi_app_stimulus_response_obj.newdocument(); % order of operations matters! superclasses last
				ndi_app_stimulus_response_obj.experiment.database.add(newdoc);
		end; % savevalidinterval()

		function b = clearvalidinterval(ndi_app_stimulus_response_obj, ndi_epochset_obj)
			% CLEARVALIDINTERVAL - clear all 'validinterval' records for an NDI_EPOCHSET from experiment database
			% 
			% B = CLEARVALIDINTERVAL(NDI_APP_STIMULUS_RESPONSE_OBJ, NDI_EPOCHSET_OBJ)
			%
			% Clears all valid interval entries from the experiment database for object NDI_EPOCHSET_OBJ.
			%
			% Returns 1 on success, 0 otherwise.
			%
			% See also: NDI_APP_STIMULUS_RESPONSE/MARKVALIDINTERVAL, NDI_APP_STIMULUS_RESPONSE/SAVEALIDINTERVAL, ...
			%      NDI_APP_STIMULUS_RESPONSE/LOADVALIDINTERVAL 

				[vi,mydoc] = ndi_app_stimulus_response_obj.loadvalidinterval(ndi_epochset_obj);

				if ~isempty(mydoc),
					ndi_app_stimulus_response_obj.experiment.database.remove(mydoc);
				end

		end % clearvalidinteraval()

		function [vi,mydoc] = loadvalidinterval(ndi_app_stimulus_response_obj, ndi_epochset_obj)
			% LOADVALIDINTERVAL - Load all valid interval records from experiment database
			%
			% [VI,MYDOC] = LOADVALIDINTERVAL(NDI_APP_STIMULUS_RESPONSE_OBJ, NDI_EPOCHSET_OBJ)
			%
			% Loads stored valid interval records generated by NDI_APP_STIMULUS_RESPONSE/MAKEVALIDINTERVAL
			%
			...
			%
				...
				searchq = cat(2,ndi_app_stimulus_response_obj.searchquery(), ...
				...
				mydoc = ndi_app_stimulus_response_obj.experiment.database.search(searchq);
		...
		function [intervals] = identifyvalidintervals(ndi_app_stimulus_response_obj, ndi_epochset_obj, timeref, t0, t1)
			% IDENTIFYVALIDINTERVAL - identify valid region within an interval
			%
			% INTERVALS = IDENTIFYVALIDINTERVALS(NDI_APP_STIMULUS_RESPONSE_OBJ, NDI_EPOCHSET_OBJ, TIMEREF, T0, T1)
			%
			% Examines whether there is a stored 'validinterval' variable by the app 'ndi_app_stimulus_response' for
			...
			%
				...
				vi = ndi_app_stimulus_response_obj.loadvalidinterval(ndi_epochset_obj);
				if isempty(vi),
					return;
				end;
				for i=1:size(vi,1),
					...
					%    Can we project the marked valid region into this timeref?
						interval_t0_timeref = ndi_timereference(ndi_app_stimulus_response_obj.experiment, vi(i).timeref_structt0);
						interval_t1_timeref = ndi_timereference(ndi_app_stimulus_response_obj.experiment, vi(i).timeref_structt1);
						[epoch_t0_out, epoch_t0_timeref, msg_t0] = ...
								ndi_app_stimulus_response_obj.experiment.syncgraph.time_convert(interval_t0_timeref, ...
									vi(i).t0, timeref.referent, timeref.clocktype);
						[epoch_t1_out, epoch_t1_timeref, msg_t1] = ...
								ndi_app_stimulus_response_obj.experiment.syncgraph.time_convert(interval_t1_timeref, ...
