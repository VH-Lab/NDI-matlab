classdef nsd_app_spikeextractor < nsd_app
	...
	methods
		...
		function parameters = load_parameters(nsd_app_spikeextractor_obj, nsd_probe_obj)
			% LOADSPIKES - Load all spikewaves records from experiment database
			%
			% SW = LOADSPIKES(NSD_APP_SPIKEEXTRACTOR_OBJ, NSD_PROBE_OBJ, EXTRACTION_NAME)
			%
			% Loads stored spikewaves generated by NSD_APP_SPIKEEXTRACTOR/SPIKE_EXTRACT_PROBES
			%
				parameters_searchq = cat(2, nsd_app_spikeextractor_obj.searchquery(), ...
					{'document_class.class_name','extraction_parameters'});
				parameters_searchq = cat(2, parameters_searchq, nsd_probe_obj.searchquery());
				%parameters_searchq = cat(2, parameters_searchq, ...
				%	{'spike_extraction.extraction_name',extraction_name});
				docs = nsd_app_spikeextractor_obj.experiment.database.search(parameters_searchq);

				% TODO How to get them in order? maybe add epoch_number to spikes and times nsd_doc
				if ~isempty(docs)
					% TODO make sure multiple epochs work
					for i=1:numel(docs)
						parameters_doc = nsd_app_spikeextractor_obj.experiment.database.read(docs{i}.doc_unique_id)
						spikewaves_binarydoc_fid = nsd_app_spikeextractor_obj.experiment.database.read(parameters_doc);
						waveforms = [];
