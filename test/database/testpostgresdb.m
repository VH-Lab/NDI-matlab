...
mypostgresdb = ndi_postgresdb("matthewgonzalgo","matthewgonzalgo","1Password!");
mypostgresdb.db.Message
disp('========== mypostgres made =============');
% json_info = '["one", "two", "three"]'
% matlab_struct = jsondecode(json_info)
%class(matlab_struct)
% Some issue with ndi_document handling non struct types?

s = struct('x',{'a','b'},'list','abc')
mydoc = ndi_document(s)

q = ndi_query('list','contains_string','abc','')
...
disp("================== 2 ========================");
q2 = ndi_query('id','exact_string','a7bb1907805e4b75af16fcb96c62ab14','');
ndiquery_to_sql(mypostgresdb, q2.to_searchstructure)

disp("================== 3 ========================");
q3 = ndi_query('id','exact_number','matlab','');
