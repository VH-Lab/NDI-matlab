classdef blankSessionKjnielsenlab < matlab.unittest.TestCase
    ...
    methods (TestMethodSetup)
        function setupBlankSession(testCase)
            testCase.SessionPath = fullfile(tempdir(), 'NDI', 'test_blankSessionKjnielsenlab');
            if isfolder(testCase.SessionPath)
                rmdir(testCase.SessionPath, 's');
            end
            mkdir(testCase.SessionPath);
            testCase.Session = ndi.setup.lab('kjnielsenlab', 'exp1', testCase.SessionPath);
        end
    end
    ...
    methods (Test)
        function testBlankSessionKjnielsenlab(testCase)
            artifactDir = fullfile(tempdir(), 'NDI', 'symmetryTest', 'matlabArtifacts', 'session', 'blankSessionKjnielsenlab', 'testBlankSessionKjnielsenlab');
