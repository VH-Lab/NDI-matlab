classdef blankSessionRayolab < matlab.unittest.TestCase
    ...
    methods (TestMethodSetup)
        function setupBlankSession(testCase)
            testCase.SessionPath = fullfile(tempdir(), 'NDI', 'test_blankSessionRayolab');
            if isfolder(testCase.SessionPath)
                rmdir(testCase.SessionPath, 's');
            end
            mkdir(testCase.SessionPath);
            testCase.Session = ndi.setup.rayolab('exp1', testCase.SessionPath);
        end
    end
    ...
    methods (Test)
        function testBlankSessionRayolab(testCase)
            artifactDir = fullfile(tempdir(), 'NDI', 'symmetryTest', 'matlabArtifacts', 'session', 'blankSessionRayolab', 'testBlankSessionRayolab');
