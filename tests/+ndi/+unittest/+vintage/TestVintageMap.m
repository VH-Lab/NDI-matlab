...
%             ndi.vintage.objectClass, ndi.vintage.edge, ndi.vintage.field.
    ...
    methods (Test)
        ...
        function testTheMapDeclaresItsSizeAndIsWellFormed(testCase)
            ...
            m = ndi.vintage.map();
            testCase.verifyEqual(numel(m), 8, sprintf( ...
                ['ndi.vintage.map declares %d concept(s); this test was ' ...
                 'written against 8 (daqsystem, filenavigator, ' ...
                 'daqmetadatareader, daqreader, syncgraph, syncrule, ' ...
                 'element, element_epoch)'], numel(m)));
            for i = 1:numel(m)
                ...
                %
                % NARROWED, NOT WEAKENED, when element_epoch joined the map
                % (2026-08-17). This used to demand a locator from EVERY
                % row, which held only while every row was an NDI OBJECT
                % type. `element_epoch` is not one: it is data reached
                % through an element, so v1 encodes no `ndi_*_class` on it
                % and there is nothing for V_eta to have moved. The
                % v1 side now selects -- a row that names an
                % `object_field` is claiming v1 stored a MATLAB class on
                % that document, and THAT is the claim which obliges a
                % V_eta home. A row with no object_field is exempt, and a
                % row with one and no home still fails.
                if isempty(m(i).object_field)
                    testCase.verifyEmpty(m(i).object_edge, sprintf( ...
                        ['%s: declares a V_eta object edge but no v1 ' ...
                         'object_field -- one side of the pair is missing'], ...
                        m(i).concept));
                    testCase.verifyEmpty(m(i).object_assertion, sprintf( ...
                        ['%s: declares a V_eta object assertion but no v1 ' ...
                         'object_field -- one side of the pair is missing'], ...
                        m(i).concept));
                    continue;
                end
