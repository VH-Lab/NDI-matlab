classdef TestBlobFixture < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testParallelWorkersProduceIdenticalChunks(testCase)
            ...
            % Match up level docs by (level index, reduction) then
            % compare each chunk.bin_# byte-for-byte.
            levelsS = Ss.database_search( ...
                ndi.query('','depends_on', ...
                    'lightsheetZarrPyramid_id', pS.id()) & ...
                ndi.query('','isa','lightsheetZarrLevel'));
            levelsP = Sp.database_search( ...
                ndi.query('','depends_on', ...
                    'lightsheetZarrPyramid_id', pP.id()) & ...
                ndi.query('','isa','lightsheetZarrLevel'));
            testCase.verifyEqual(numel(levelsS), numel(levelsP), ...
                'Level count must match between serial and parallel');

            keyOf = @(L) sprintf('%d-%s', ...
                L.document_properties.lightsheetZarrLevel.level, ...
                L.document_properties.lightsheetZarrLevel.reduction_function);
            keysS = cellfun(keyOf, levelsS, 'UniformOutput', false);
            keysP = cellfun(keyOf, levelsP, 'UniformOutput', false);

            for i = 1:numel(levelsS)
                j = find(strcmp(keysP, keysS{i}), 1);
                testCase.verifyNotEmpty(j, ...
                    sprintf('No matching parallel level for %s', keysS{i}));
                lS = levelsS{i};
                lP = levelsP{j};
                nc = lS.document_properties.lightsheetZarrLevel.n_chunks_stored;
                testCase.verifyEqual( ...
                    lP.document_properties.lightsheetZarrLevel.n_chunks_stored, ...
                    nc, 'n_chunks_stored differs');
                for k = 1:nc
                    nm = sprintf('chunk.bin_%d', k);
                    fs = Ss.database_openbinarydoc(lS, nm);
                    bs = fread(fs.fid, Inf, 'uint8=>uint8');
                    Ss.database_closebinarydoc(fs);
                    fp = Sp.database_openbinarydoc(lP, nm);
                    bp = fread(fp.fid, Inf, 'uint8=>uint8');
                    Sp.database_closebinarydoc(fp);
                    testCase.verifyEqual(bs, bp, ...
                        sprintf('chunk %s of level %s differs ' ...
