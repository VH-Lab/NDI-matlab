classdef buildSessionNDRAxon < matlab.unittest.TestCase
    properties
        Session
    end
    ...
    methods (TestMethodSetup)
        function buildSessionSetup(testCase)
            % BUILDSESSIONSETUP - Build an example Axon session in a temporary directory
            ...
            source_dir = fullfile(ndr.fun.ndrpath, 'example_data');
            filename = 'example.abf';
            ...
            probemap_filename = fullfile(dirname, 'example.epochprobemap.ndi');
            if ~exist(probemap_filename,'file')
                ...
                fprintf(fid,'ctx\t1\tn-trode\taxon1:ai1\tanteater27@nosuchlab.org\n');
                fclose(fid);
            end
            ...
            % Add acquisition daqsystem (axon)
            dt = ndi.file.navigator(testCase.Session, {'#.abf', '#.epochprobemap.ndi'},'ndi.epoch.epochprobemap_daqsystem',{'(.*)epochprobemap.ndi'});
            dev1 = ndi.daq.system.mfdaq('axon1',dt,ndi.daq.reader.mfdaq.ndr('abf'));
