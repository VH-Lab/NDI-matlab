    ...
    methods (Test)
        ...
        function testStimulusPresentationRefusalsByReason(testCase)
            ...
            fprintf(['    epoch-fallback attribution of the refused (report-only, #31):\n' ...
                     '      UNSCOPED (epoch id pooled across all sessions):\n' ...
                     '        exactly one recorded subject: %d\n' ...
                     '        none (subject-less epoch):     %d\n' ...
                     '        several (ambiguous):           %d\n' ...
                     '      SESSION-SCOPED (epoch within the presentation''s own session):\n' ...
                     '        exactly one recorded subject: %d\n' ...
                     '        none (subject-less epoch):     %d\n' ...
                     '        several (ambiguous):           %d\n'], ...
                r.epoch_fallback_one, r.epoch_fallback_zero, r.epoch_fallback_multi, ...
                r.epoch_fallback_scoped_one, r.epoch_fallback_scoped_zero, ...
                r.epoch_fallback_scoped_multi);
            % READING IT: if SESSION-SCOPED collapses the unscoped `several` into
            % `one`, the ambiguity was a cross-session epoch-name pooling
            % artifact and fork 1 (attribute via the epoch's recorded subject)
            % works. If `several` persists under scoping, the epoch genuinely
            % records multiple subjects -> the signed multi-subject model (one
            % shared timed_sequence + N reference manipulations).
