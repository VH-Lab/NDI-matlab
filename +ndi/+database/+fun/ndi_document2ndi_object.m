function o = ndi_document2ndi_object(ndi_document_obj, ndi__session_obj)
...
% 
 ...
 % TODO: what if ndi__session_obj does not match the current session?

if ~isa(ndi_document_obj, 'ndi.document'),
	% try to look it up
	mydoc = ndi__session_obj.database_search(ndi.query('ndi_document.id','exact_string',ndi_document_obj,''));
	if numel(mydoc)==1,
...
o = eval([obj_string '(ndi__session_obj, ndi_document_obj);']);
