    ...
    methods
        function obj = AuthorDataGUI(parentAppHandle, uiParentContainerForAuthors)
            ...
            if ~isfolder(obj.ResourcesPath)
                ...
                fallbackPath = fullfile(projectRootGuess, 'resources', 'icons');
                if isfolder(fallbackPath)
                    obj.ResourcesPath = fallbackPath;
                    fprintf(1, 'Info (AuthorDataGUI): Using fallback ResourcesPath: %s\n', obj.ResourcesPath);
                else
                     fprintf(2, 'Warning (AuthorDataGUI): Fallback ResourcesPath also does not exist: %s\n', fallbackPath);
                end
            end
        ...
        function addAffiliationButtonPushed(obj)
            ...
            if isNewOrg
                 ...
                 S_org_returned = obj.ParentApp.openOrganizationForm(S_org); 
