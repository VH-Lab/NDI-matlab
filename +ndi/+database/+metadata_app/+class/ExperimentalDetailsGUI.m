classdef ExperimentalDetailsGUI < handle
    ...
    properties (Access = public)
        ...
        UIBaseContainer % This will now be obj.ExperimentDetailsPanel, created by this class
        ...
        ExperimentDetailsPanel matlab.ui.container.Panel % Panel where detailed UI is built
    end
    ...
    methods
        ...
        function obj = ExperimentalDetailsGUI(parentAppHandle, experimentDetailsTabHandle) % Accepts ExperimentDetailsTab
            ...
            obj.createExperimentDetailsTabBaseLayout(experimentDetailsTabHandle); % Create base structure
            obj.createExperimentalDetailsUIComponents(); % Populate the self-created panel
        end
        
        % NEW METHOD to create the base layout for the Experiment Details tab content
        function createExperimentDetailsTabBaseLayout(obj, experimentDetailsTabHandle)
            % experimentDetailsTabHandle is app.ExperimentDetailsTab passed from MetadataEditorApp
            obj.ExperimentDetailsGridLayout = uigridlayout(experimentDetailsTabHandle, [2 1], 'RowHeight', {60, '1x'}, 'Padding', [10 20 10 10]); %
            obj.ExperimentDetailsLabel = uilabel(obj.ExperimentDetailsGridLayout, 'Text', 'Experiment Details', 'HorizontalAlignment', 'center', 'FontSize', 18, 'FontWeight', 'bold'); %
            obj.ExperimentDetailsLabel.Layout.Row=1; obj.ExperimentDetailsLabel.Layout.Column=1; %
            obj.ExperimentDetailsPanel = uipanel(obj.ExperimentDetailsGridLayout, 'BorderType', 'none'); %
            obj.ExperimentDetailsPanel.Layout.Row=2; obj.ExperimentDetailsPanel.Layout.Column=1; %
            
            % Set UIBaseContainer to the newly created ExperimentDetailsPanel.
            obj.UIBaseContainer = obj.ExperimentDetailsPanel;
        end
        ...
        function createExperimentalDetailsUIComponents(obj)
            % This method now uses obj.UIBaseContainer, which is obj.ExperimentDetailsPanel
            ...
            obj.SelectTechniqueDropDown.Layout.Row = 4; %
            obj.SelectTechniqueDropDown.Layout.Column = 5; %
            ...
            obj.SelectedTechniquesListBox.Layout.Row = [7 8]; %
            obj.SelectedTechniquesListBox.Layout.Column = 5;  %
        ...
        function drawExperimentalDetails(obj)
            ...
            if isfield(dsStruct, 'TechniquesEmployed')  %
                obj.setSelectedTechniques(dsStruct.TechniquesEmployed); %
            else
        ...
        function addTechniqueButtonPushed(obj, event)
            ...
            if ~isempty(idx_for_display) %
                ...
                if idx_for_display <= numel(obj.SelectTechniqueDropDown.Items) %
                    ...
                    obj.ParentApp.alert('Internal error: Mismatch in technique dropdown items. Please report.','Error'); %
                    return; %
                end
            else
                 obj.ParentApp.alert(sprintf('Selected technique ID "%s" not found in internal data. Please re-select from the list.', techniqueID), 'Selection Error'); %
                 return; %
            end
        ...
        function updateTechniquesEmployedFromListBox(obj)
            ...
            for i = 1:numel(listBoxItems_Display) %
                ...
                if ~isempty(match) %
                    ...
                    obj.populateTechniqueDropdown(categoryNamePart); %
                    ...
                    obj.SelectTechniqueCategoryDropDown.Value = originalCatDropdownVal; %
                    obj.populateTechniqueDropdown(originalCatDropdownVal); %
                    obj.SelectTechniqueDropDown.Value = originalTechDropdownValue; %

                    if ~isempty(idFound) && ~strcmp(idFound,"") %
                        newTechniqueIDs{end+1} = idFound; %
%#ok<AGROW> %
                    else
        ...
        function setSelectedTechniques(obj, techniques_data_ids) 
            ...
            if ~isempty(techniques_data_ids) %
                ...
                % originalTechItems = obj.SelectTechniqueDropDown.Items; % Not needed for restore if populateTechniqueDropdown used
                % originalTechItemsData = obj.SelectTechniqueDropDown.ItemsData; % Not needed

                for cat_idx = 1:numel(allCategories) %
                    categoryName = allCategories{cat_idx}; %
                    if strcmp(categoryName, "Select Category"), continue; end %
                    
                    obj.populateTechniqueDropdown(categoryName); %
                ...
                obj.populateTechniqueDropdown(originalCatVal); % Re-populate for original category
                obj.SelectTechniqueDropDown.Value = originalTechVal; % Attempt to restore selection
        ...
        function missingFields = checkRequiredFields(obj)
            ...
            % Check DataType
            if isfield(requiredFields, 'DataType') && requiredFields.DataType
                selectedDataTypes = obj.getDataType();
                if isempty(selectedDataTypes)
                    missingFields(end+1) = obj.ParentApp.getFieldTitle('DataType');
                    obj.ParentApp.highlightLabelForRequiredField('DataType');
                else
                    obj.ParentApp.resetLabelForRequiredField('DataType');
                end
            end
            % Add checks for other required fields specific to this GUI if any
        end
        ...
        function markRequiredFields(obj)
            ...
            % DataTypeTreeLabel is already marked with (*) in createExperimentalDetailsUIComponents
            % if isfield(requiredFields, 'DataType') && requiredFields.DataType
            %     if ~contains(obj.DataTypeTreeLabel.Text, requiredSymbol)
            %         obj.DataTypeTreeLabel.Text = [obj.DataTypeTreeLabel.Text ' ' requiredSymbol];
            %     end
            %     obj.DataTypeTreeLabel.Tooltip = "Required";
            % end
            % Mark other required fields if any
        end
