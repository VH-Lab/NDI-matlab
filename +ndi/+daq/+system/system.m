...
%  This is an abstract class that is overridden by specific devices.
	...
	methods
		function obj = ndi.daq.system(name,thefilenavigator,thedaqreader,thedaqmetadatareader)
			...
			%
				...
				if nargin==2 & isa(name,'ndi.session.session') & isa(thefilenavigator,'ndi.document');
					...
					docs = session.database_search(ndi.query('ndi_document.id','exact_string',daqreader_id,''));
					if numel(docs)~=1,
						error(['Could not find daqreader document with id ' daqreader_id '.']);
					end;
					daqreader_doc = docs{1};
					docs = session.database_search(ndi.query('ndi_document.id','exact_string',filenavigator_id,''));
					if numel(docs)~=1,
						error(['Could not find daqreader document with id ' daqreader_id '.']);
					end;
					...
					for i=1:numel(D),
						metadatadocs{i} = session.database_search(ndi.query('ndi_document.id','exact_string',D{i},''));
		...
		function ndi_daqsystem_obj = set_daqmetadatareader(ndi_daqsystem_obj, thedaqmetadatareaders)
			% SET_DAQMETADATAREADER - set the cell array of ndi.daq.metadatareader.metadatareader objects
			%
			...
			% ndi.daq.metadatareader.metadatareader as a superclass.
			%
				...
				for i=1:numel(thedaqmetadatareaders),
					if ~isa(thedaqmetadatareaders{i},'ndi.daq.metadatareader.metadatareader'),
						error(['Element ' int2str(i) ' of THEDAQMETADATAREADERS is not of type ndi.daq.metadatareader.metadatareader.']);
					end;
				end;
		...
		function metadata = getmetadata(ndi_daqsystem_obj, epoch, channel)
			...
			% CHANNEL. CHANNEL indicates the number of the ndi.daq.metadatareader.metadatareader to use 
			% to obtain the data.
		...
		function ndi_document_obj_set = newdocument(ndi_daqsystem_obj)
			...
			% Creates a set of documents that describe an ndi.daq.system.
				...
				ndi_document_obj_set{3} = ndi.document('ndi_document_daqsystem.json',...
		...
		function sq = searchquery(ndi_daqsystem_obj)
			...
			%
				sq = ndi.query({'ndi_document.id',ndi_daqsystem_obj.id(), ...  % really this is the only one necessary
						'ndi_document.name', ndi_daqsystem_obj.name, ...
						'ndi_document.session_id', ndi_daqsystem_obj.session.id()});
