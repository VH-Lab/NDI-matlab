classdef base < ndi.ido & ndi.epoch.epochset & ndi.documentservice
% ndi.element.base - define or examine a element in the session
%
	properties (GetAccess=public, SetAccess=protected)
		...
		subject_id   % ID of the subject that is related to the ndi.element.base
	end; % properties

	methods
		function ndi_element_obj = base(varargin)
			% NDI_ELEMENT_OBJ = ndi.element.base - creator for ndi.element.base
			%
			% NDI_ELEMENT_OBJ = ndi.element.base(NDI_SESSION_OBJ, ELEMENT_NAME, ELEMENT_REFERENCE, ...
			%        ELEMENT_TYPE, UNDERLYING_EPOCHSET, DIRECT, [SUBJECT_ID])
			%    or
			% NDI_ELEMENT_OBJ = ndi.element.base(NDI_SESSION_OBJ, ELEMENT_DOCUMENT)
			%
			% Creates an ndi.element.base object, either from a name and and associated ndi.probe object,
			...
			%
				if numel(varargin)>=6,
					% first type
					ndi_element_class = 'ndi.element.base';
					...
					if ~isempty(element_underlying_element),
						if ~isa(element_underlying_element,'ndi.element.base'),
							error(['Underlying element must be an ndi.element.base.']);
						end;
					end;
					if ~isempty(element_underlying_element),
				...
				elseif numel(varargin)==0,
					...
					element_underlying_element = [];
					direct = 0;
					warning('empty call to ndi.element.base(); did not think this would happen...');
				else,
	...
	% ndi.epoch.epochset-based methods
		...
		function b = issyncgraphroot(ndi_element_obj)
			...
			%
			% For ndi.element.base objects, this returns 0 so that underlying ndi.probe epochs are added.
				b = isempty(ndi_element_obj.underlying_element);
		end; % issyncgraphroot

		function name = epochsetname(ndi_element_obj)
			% EPOCHSETNAME - the name of the ndi.element.base object, for EPOCHNODES
			%
			% NAME = EPOCHSETNAME(NDI_ELEMENT_OBJ)
			%
			% Returns the object name that is used when creating epoch nodes.
			%
			% For ndi.element.base objects, this is ndi.element.base/ELEMENTSTRING. 
		...
		function ec = epochclock(ndi_element_obj, epoch_number)
			...
			%
			% The ndi.element.base class always returns the clock type(s) of the element it is based on
			%
		...
		function [cache,key] = getcache(ndi_element_obj)
			% GETCACHE - return the NDI_CACHE and key for ndi.element.base
			%
			% [CACHE,KEY] = GETCACHE(NDI_ELEMENT_OBJ)
			%
			% Returns the CACHE and KEY for the ndi.element.base object.
			%
		...
		function et = buildepochtable(ndi_element_obj)
			% BUILDEPOCHTABLE - build the epoch table for an ndi.element.base
			%
		...
		%% unique ndi.element.base methods
		...
		function elementstr = elementstring(ndi_element_obj)
			...
			%
			% For ndi.element.base objects, this is the string 'element: ' followed by its name
			% 
		...
		function [ndi_element_obj, epochdoc] = addepoch(ndi_element_obj, epochid, epochclock, t0_t1)
			...
			%   
				epochdoc = [];
				if ndi_element_obj.direct,
					error(['Cannot add external observations to an ndi.element.base that is directly based on ndi.probe.*']);
				end;
				E = ndi_element_obj.session;
				if ~isempty(E),
					elementdoc = E.database_search(ndi_element_obj.searchquery());
					if isempty(elementdoc),
						error(['ndi.element.base is not part of the database.']);
					elseif numel(elementdoc)>1,
						error(['More than one document corresponds to this ndi.element.base; shouldn''t happen.']);
					else,
		...
		function [et_added, epochdocs] = loadaddedepochs(ndi_element_obj)
			% LOADADDEDEPOCHS - load the added epochs from an ndi.element.base
			%
		...
		function element_doc = load_element_doc(ndi_element_obj)
			...
			%
			% Load an ndi.document that is based on the ndi.element.base object.
			%
		...
		function element_ref = doc_unique_id(ndi_element_obj)
			% DOC_UNIQUE_REF - return the document unique reference for an ndi.element.base object
			%
		...
		function element_id = id(ndi_element_obj)
			% ID - return the document unique identifier for an ndi.element.base object
			%
		...
		function element_docs = load_all_element_docs(ndi_element_obj)
			% LOAD_ALL_ELEMENT_DOCS - load all of the ndi.element.base objects from an session database
			%
			% ELEMENT_DOCS = LOAD_ALL_ELEMENT_DOCS(NDI_ELEMENT_OBJ)
			%
			% Loads the ndi.document that is based on the ndi.element.base object and any associated
			% epoch documents.
			%
				element_doc = ndi_element_obj.load_element_doc();
				if ~isempty(element_doc),
