classdef MetadataEditorApp < matlab.apps.AppBase
    ...
    methods (Access = public) 
        ...
        function setCheckedNodesFromData(app, treeHandle, data)
            ...
            elseif iscell(data) && ~isempty(data) && ~all(cellfun(@ischar, data)) 
                try data = cellfun(@char, data, 'UniformOutput', false);
                catch; data = {}; end
            end
    ...
    methods (Access = public) % Load/save and data object interaction 
        ...
        function resetFigureNameIn(app, name, numSeconds)
            if ~isempty(app.Timer) && isvalid(app.Timer), stop(app.Timer);
                delete(app.Timer); app.Timer = []; end
        ...
        function updateFigureName(app, name) 
            ...
            if ~isempty(app.Timer) && isvalid(app.Timer), stop(app.Timer); delete(app.Timer);
                app.Timer = []; end
        end
        ...
        function performActualFormReset(app)
            if isprop(app, 'AuthorData') && ismethod(app.AuthorData, 'ClearAll'), app.AuthorData.ClearAll();
                app.AuthorData.addDefaultAuthorEntry(); end
            if isprop(app, 'SubjectData') && ismethod(app.SubjectData,'ClearAll'), app.SubjectData.ClearAll();
            end
            if isprop(app, 'ProbeData') && ismethod(app.ProbeData,'ClearAll'), app.ProbeData.ClearAll();
            end
        ...
        function ResetFormButtonPushed(app, event)
            answer = uiconfirm(app.NDIMetadataEditorUIFigure, ...
                sprintf(['Are you sure you want to reset the form? All unsaved changes will be lost ' ...
                         'and the form will be repopulated with initial data\n' ...
                         'from the NDI entity or to a blank state.']), ...
                'Confirm Reset', ...
    ...
    methods (Access = private)
        function createComponents(app)
            ...
            app.IntroductionTextLabel = uilabel(app.GridLayout_Step0_C2, 'Text', ...
                ...
                ['If you ever need assistance with any of the form elements, simply ' ...
                 'hover over the respective item to access helpful information. For any further queries, shoot us an email at info@walthamdatascience.com and we''ll get right back to you! Thank you for choosing our app to help you manage your data. Let''s get started on this journey together!']}, ...
    ...
    methods (Access = public) 
        function startupFcn(app, datasetObject, debugMode)
            ...
            app.AuthorDataGUI_Instance = ndi.database.metadata_app.class.AuthorDataGUI(app, app.AuthorsTab); 
            app.AuthorDataGUI_Instance.initialize();
            
            app.DatasetDetailsGUI_Instance = ndi.database.metadata_app.class.DatasetDetailsGUI(app, app.DatasetDetailsTab); 
            app.DatasetDetailsGUI_Instance.initialize();
            
            app.ExperimentalDetailsGUI_Instance = ndi.database.metadata_app.class.ExperimentalDetailsGUI(app, app.ExperimentDetailsTab); 
            app.ExperimentalDetailsGUI_Instance.initialize();
            
            app.SubjectInfoGUI_Instance = ndi.database.metadata_app.class.SubjectInfoGUI(app, app.SubjectInfoTab);
            app.SubjectInfoGUI_Instance.initialize();
            
            app.ProbeDataGUI_Instance = ndi.database.metadata_app.class.ProbeDataGUI(app, app.ProbeInfoTab); 
