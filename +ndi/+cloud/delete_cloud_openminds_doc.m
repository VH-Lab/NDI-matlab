function [size, session_id, openminds_doc] =  delete_cloud_openminds_doc(auth_token,dataset_id)
...
size = 0;
[status, response, documents_summary] = ndi.cloud.api.documents.get_documents_summary(dataset_id, auth_token);
...
for i = 1:numel(openminds_documents_id)
    [status, response, document] = ndi.cloud.api.documents.get_documents(dataset_id, openminds_documents_id{i}, auth_token);
...
[statu, dataset] = ndi.cloud.api.datasets.post_bulk_delete(dataset_id, openminds_documents_id, auth_token);
