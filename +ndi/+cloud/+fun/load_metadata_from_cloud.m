...
for i = 1:numel(openminds_documents)
    if strcmp(openminds_documents{i}.document_properties.openminds.matlab_type, 'openminds.core.products.Dataset')
        dataset_doc{1} = openminds_documents{i};
        d_f = dataset_doc{1, 1}.document_properties.openminds.fields;
        break
    end
end

for i = 1:numel(openminds_documents)
    if strcmp(openminds_documents{i}.document_properties.openminds.matlab_type, 'openminds.core.products.DatasetVersion')
...
for i = 1:numel(author_doc_id)
    for j = 1:numel(openminds_documents)
        if contains(author_doc_id{i},openminds_documents{j}.document_properties.base.id)
...
for i = 1:numel(experimentalApproach_id)
    ...
    experimentalApproach_name{i} = experimentalApproach.document_properties.openminds.fields.name;
end
...
for i = 1:numel(TechniquesEmployed_id)
    ...
    TechniquesEmployed_name{i} = TechniquesEmployed.document_properties.openminds.fields.name;
end
...
for i = 1:numel(funding_id)
    ...
    funding.awardTitle = funding_doc.document_properties.openminds.fields.awardTitle;
    funding.awardNumber = funding_doc.document_properties.openminds.fields.awardNumber;
    funder_doc = ndi.cloud.fun.search_id(funding_doc.document_properties.openminds.fields.funder{1},openminds_documents);
    funding.funder = funder_doc.document_properties.openminds.fields.fullName;
end
...
for i = 1:numel(license_id)
    ...
    datasetInformation.License = short_name_to_names(license_doc.document_properties.openminds.fields.shortName);
end