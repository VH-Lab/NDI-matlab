classdef navigator < ndi.ido & ndi.epoch.epochset.param & ndi.documentservice
	...
	properties (GetAccess=public, SetAccess=protected)
		session                    % The ndi.session.session to be examined (handle)
	...
	methods
		...
		%
		% Inputs:
		%      SESSION: an ndi.session.session
		% Optional inputs:
		...
		%
			...
			if nargin==2 & isa(session_,'ndi.session.session') & isa(fileparameters_,'ndi.document'),
			...
			if ~isempty(session_),
				if ~isa(session_,'ndi.session.session'),
					error(['experiement must be an ndi.session.session object']);
				else,
		...
		function id = epochid(ndi_filenavigator_obj, epoch_number, epochfiles)
			...
			% 
				...
				if exist(eidfname,'file'),
					...
					id = ['epoch_' ndi.ido.ndi_unique_id()];
		...
		function thepath = path(ndi_filenavigator_obj)
			...
			%
			% Returns the path of the ndi.session.session associated with the ndi.file.navigator object
			% NDI_FILENAVIGATOR_OBJ.
			%
				if ~isa(ndi_filenavigator_obj.session,'ndi.session.session'),
					error(['No valid ndi.session.session associated with this filenavigator object.']);
				else,
