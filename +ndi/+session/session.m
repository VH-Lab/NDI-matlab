classdef session < handle % & ndi.documentservice & % ndi.ido Matlab does not allow these subclasses because they are not handles
	...
	methods
		function ndi__session_obj = session(reference)
			...
			%   ndi.session.session/GETPATH, ndi.session.session/GETREFERENCE
				...
				ndi__session_obj.identifier = ndi.ido.ndi_unique_id();
				ndi__session_obj.syncgraph = ndi.time.syncgraph(ndi__session_obj);
				ndi__session_obj.cache = ndi.cache();
		end
		...
		function refstr = unique_reference_string(ndi__session_obj)
			...
			% and the UNIQUE_REFERENCE property of NDI_SESSION_OBJ, joined with a '_'.
				...
				dbstack
				refstr = [ndi__session_obj.reference '_' ndi__session_obj.identifier];
		...
		function ndi__session_obj = daqsystem_rm(ndi__session_obj, dev)
			...
			% See also: DAQSYSTEM_ADD, ndi.session.session
				...
				daqsys = ndi__session_obj.daqsystem_load('name',dev.name);
				if ~isempty(daqsys) 
					if ~iscell(daqsys),
						daqsys = {daqsys};
					end;
					docs = ndi__session_obj.database_search(daqsys{1}.searchquery());
					for k=1:numel(docs), % should be 1 only, but keep deleting even if not
						for i=1:numel(docs{k}.document_properties.depends_on),
							dochere = ndi__session_obj.database_search(...
								ndi.query('ndi_document.id', 'exact_string', docs{k}.document_properties.depends_on(i).value, ''));
							ndi__session_obj.database_rm(dochere);
						end;
						ndi__session_obj.database_rm(docs); % database_rm can process single or a cell list of ndi_document_obj(s)
					end;
				else
		...
		function dev = daqsystem_load(ndi__session_obj, varargin)
			...
			%
				...
				q2 = ndi.query('ndi_document.session_id','exact_string',ndi__session_obj.id(),'');
				q = q1 & q2;
				if numel(varargin)>0,
				...
				dev_doc = ndi__session_obj.database_search(q);
				% dev is cell list of ndi.document objects
				for i=1:numel(dev_doc),
					dev{i} = ndi.database.fun.document2ndi_object(dev_doc{i},ndi__session_obj);
				end;
		...
		function ndi__session_obj = daqsystem_clear(ndi__session_obj)
			...
			% Be sure you mean it!
			%
				dev = ndi__session_obj.daqsystem_load('name','(.*)');
				if ~isempty(dev) & ~iscell(dev),
					dev = {dev};
				end;
				for i=1:numel(dev),
					dev{i},
					ndi__session_obj = ndi__session_obj.daqsystem_rm(dev{i});
				end;
		...
		function ndi_document_obj = newdocument(ndi__session_obj, document_type, varargin)
			...
			%
			% Example: mydoc = ndi__session_obj.newdocument('ndi_document','ndi_document.name','myname');
			%
				if nargin<2,
					document_type = 'ndi_document.json';
				end
				inputs = cat(2,varargin,{'ndi_document.session_id', ndi__session_obj.id()});
				ndi_document_obj = ndi.document(document_type, inputs);
		end; %newdocument()

		function sq = searchquery(ndi__session_obj)
			...
			% by this session.
			%
			% SQ = {'ndi_document.session_id', ndi__session_obj.id()};
			% 
			% Example: mydoc = ndi__session_obj.newdocument('ndi_document','ndi_document.name','myname');
			%
				sq = {'ndi_document.session_id', ndi__session_obj.id()};
			...
			% See also: DATABASE_RM, ndi.session.session, ndi.database, ndi.session.session/SEARCH
				...
				if iscell(document),
					for i=1:numel(document),
						ndi__session_obj.database_add(document{i});
					end;
					return;
				end;
				...
				ndi__session_obj.database.add(document);
		end; % database_add()
			...
			% See also: DATABASE_ADD, ndi.session.session
				...
				if ~iscell(doc_unique_id),
					if ischar(doc_unique_id), % it is a single doc id
						mydoc = ndi__session_obj.database_search(ndi.query('ndi_document.id','exact_string',doc_unique_id,''));
						if isempty(mydoc), % 
							if ErrIfNotFound,
				...
				if iscell(doc_unique_id),
					dependent_docs = ndi.database.fun.findalldependencies(ndi__session_obj,[],doc_unique_id{:});
					if numel(dependent_docs)>1,
						warning(['Also deleting ' int2str(numel(dependent_docs)) ' dependent docs.']);
					end;
					for i=1:numel(dependent_docs),
						ndi__session_obj.database.remove(dependent_docs{i});
					end;
					for i=1:numel(doc_unique_id), 
						ndi__session_obj.database.remove(doc_unique_id{i});
					end;
				else,
		...
		function ndi_document_obj = database_search(ndi__session_obj, searchparameters)
			...
			%
				ndi_document_obj = ndi__session_obj.database.search(searchparameters);
		end % database_search();

		function database_clear(ndi__session_obj, areyousure)
			...
			%
				ndi__session_obj.database.clear(areyousure);
		end; % database_clear()
        
		function ndi_binarydoc_obj = database_openbinarydoc(ndi__session_obj, ndi_document_or_id)
			...
			% 
				ndi_binarydoc_obj = ndi__session_obj.database.openbinarydoc(ndi_document_or_id);
		end; % database_openbinarydoc

		function [ndi_binarydoc_obj] = database_closebinarydoc(ndi__session_obj, ndi_binarydoc_obj)
			...
			%
				ndi_binarydoc_obj = ndi__session_obj.database.closebinarydoc(ndi_binarydoc_obj);
		end; % closebinarydoc

		function ndi__session_obj = syncgraph_addrule(ndi__session_obj, rule)
			...
			%
				...
				update_syncgraph_in_db(ndi__session_obj);
		end; % syncgraph_addrule

		function ndi__session_obj = syncgraph_rmrule(ndi__session_obj, index)
		...
		function p = getpath(ndi__session_obj)
		...
		function obj = findexpobj(ndi__session_obj, obj_name, obj_classname)
			...
			%
				...
				if trydaqsystem,
					obj_here = ndi__session_obj.daqsystem_load('name',obj_name);
					if ~isempty(obj_here),
				...
				if tryprobelist,
					probes = ndi__session_obj.getprobes();
		...
		function probes = getprobes(ndi__session_obj, varargin)
			...
			%
				probestruct = [];
				devs = ndi__session_obj.daqsystem_load('name','(.*)');
				if ~isempty(devs),
				...
				probes = ndi.probe.fun.probestruct2probe(probestruct, ndi__session_obj);
		...
		function elements = getelements(ndi__session_obj, varargin);
			% GETELEMENTS - Return all ndi.element.element objects that are found in session database
			%
			% ELEMENTS = GETELEMENTS(NDI_SESSION_OBJ, ...)
			%
			% Examines all the database of NDI_SESSION_OBJ and returns all ndi.element.element
			% entries.
			%
			% ELEMENTS is a cell array of ndi.element.element.* objects.
			%
			...
			% 
				q_E = ndi.query(ndi__session_obj.searchquery());
				q_t = ndi.query('ndi_document.type','exact_string','ndi_element','');
				for i=1:2:numel(varargin),
					q_t = q_t & ndi.query(varargin{i},'exact_string',varargin{i+1},'');
				end;
				doc = ndi__session_obj.database_search(q_E&q_t);
				elements = {};
				for i=1:numel(doc),
					elements{i} = ndi.database.fun.document2ndi_object(doc{i}, ndi__session_obj);
				end;
		end; % getelements()
	...
	methods (Access=protected)
		function b = update_syncgraph_in_db(ndi__session_obj)
			...
			% SYNCRULEs are added or removed.
				...
				[syncgraph_doc,syncrule_doc] = ndi.time.syncgraph.load_all_syncgraph_docs(ndi__session_obj, ...
					ndi__session_obj.syncgraph.id());

				newdocs = ndi__session_obj.syncgraph.newdocument(); % generate new documents

				% now, delete old docs and add new ones

				gooddelete = 0;
				if ~isempty(syncgraph_doc),
					ndi__session_obj.database_rm(syncgraph_doc);
				end;
				if ~isempty(syncrule_doc),
					ndi__session_obj.database_rm(syncrule_doc);
				end;
				gooddelete = 1;

				% now add new docs
				ndi__session_obj.database_add(newdocs);
