classdef app < ndi.documentservice
	...
	methods
		function ndi_app_app_obj = app(varargin)
			% ndi_app_app - create a new ndi.app.app object
			%
			% NDI_APP_OBJ = ndi.app.app(SESSION)
			%
			% Creates a new ndi.app.app object that operates on the ndi.session.session
			...
			%
				...
				ndi_app_app_obj.session = session;
				ndi_app_app_obj.name = name;
		end % ndi_app_app()

		% functions related to generic variables

		function an = varappname(ndi_app_app_obj)
			...
			%
				an = ndi_app_app_obj.name;
				if ~isvarname(an),
					an = matlab.lang.makeValidName(an);
				end;
		end; % varappname ()

		function [v,url] = version_url(ndi_app_app_obj)
			...
			%
				classfilename = which(class(ndi_app_app_obj));
		...
		function c = searchquery(ndi_app_app_obj)
			...
			%
				c = {'ndi_document.session_id', ...
					ndi_app_app_obj.session.id(), ...
					'app.name',ndi_app_app_obj.varappname() };
		end;

		function ndi_document_obj = newdocument(ndi_app_app_obj)
			...
			%
				...
				[version,url] = ndi_app_app_obj.version_url();			
				...
				ndi_document_obj = ndi_app_app_obj.session.newdocument('ndi_document_app', c{:});
		end;
	end; % methods
end
