"""
NDI Ontology WBStrain - C. elegans strain names from WormBase.
...
class WBStrain(Ontology):
    ...
    def __init__(self):
        """Constructor for the WBStrain ontology object."""
        super().__init__()

    def lookup_term_or_id(self, term_or_id_or_name: str) -> Tuple[str, str, str, List[str]]:
        """
        Look up a term in WBStrain by ID or name.

        Args:
            term_or_id_or_name: Part after 'WBStrain:' prefix has been removed

        Returns:
            Tuple of (id, name, definition, synonyms)
        """
        ontology_prefix = 'WBStrain'
        ontology_name_ols = 'wbstrain'
        ...
        try:
            ...
            raise ValueError(f'Error preprocessing WBStrain lookup input "{term_or_id_or_name}": {str(e)}') from e
        ...
        try:
            ...
            raise ValueError(f'WBStrain lookup failed for {lookup_type_msg}: {str(e)}') from e
