"""
NDI Ontology CHEBI - Chemical Entities of Biological Interest.
...
class CHEBI(Ontology):
    ...
    def __init__(self):
        """Constructor for the CHEBI ontology object."""
        super().__init__()

    def lookup_term_or_id(self, term_or_id_or_name: str) -> Tuple[str, str, str, List[str]]:
        """
        Look up a term in CHEBI by ID or name.

        Args:
            term_or_id_or_name: Part after 'CHEBI:' prefix has been removed
        ...
        try:
            ...
            raise ValueError(f'Error preprocessing CHEBI lookup input "{term_or_id_or_name}": {str(e)}') from e
        ...
        try:
            ...
            raise ValueError(f'CHEBI lookup failed for {lookup_type_msg}: {str(e)}') from e
