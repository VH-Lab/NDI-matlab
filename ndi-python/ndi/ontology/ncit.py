"""
NDI Ontology NCIT - NCI Thesaurus.
...
class NCIT(Ontology):
    ...
    def __init__(self):
        """Constructor for the NCIT ontology object."""
        super().__init__()

    def lookup_term_or_id(self, term_or_id_or_name: str) -> Tuple[str, str, str, List[str]]:
        """
        Look up a term in NCIT by ID or name.

        Args:
            term_or_id_or_name: Part after 'NCIT:' prefix has been removed

        Returns:
            Tuple of (id, name, definition, synonyms)
        """
        ontology_prefix = 'NCIT'
        ontology_name_ols = 'ncit'
        ...
        try:
            ...
            raise ValueError(f'Error preprocessing NCIT lookup input "{term_or_id_or_name}": {str(e)}') from e
        ...
        try:
            ...
            raise ValueError(f'NCIT lookup failed for {lookup_type_msg}: {str(e)}') from e
