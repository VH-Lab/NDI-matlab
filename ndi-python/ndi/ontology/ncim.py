"""
NDI Ontology NCIm - NCI Metathesaurus.
...
class NCIm(Ontology):
    ...
    def __init__(self):
        """Constructor for the NCIm ontology object."""
        super().__init__()

    def lookup_term_or_id(self, term_or_id_or_name: str) -> Tuple[str, str, str, List[str]]:
        """
        Look up a term in NCIm by ID or name.

        Args:
            term_or_id_or_name: Part after 'NCIm:' prefix has been removed

        Returns:
            Tuple of (id, name, definition, synonyms)
        """
        ontology_prefix = 'NCIm'
        ontology_name_ols = 'ncim'
        ...
        try:
            ...
            raise ValueError(f'Error preprocessing NCIm lookup input "{term_or_id_or_name}": {str(e)}') from e
        ...
        try:
            ...
            raise ValueError(f'NCIm lookup failed for {lookup_type_msg}: {str(e)}') from e
