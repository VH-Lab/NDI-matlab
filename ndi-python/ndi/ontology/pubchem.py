"""
NDI Ontology PubChem - PubChem Compound.
...
class PubChem(Ontology):
    ...
    def __init__(self):
        """Constructor for the PubChem ontology object."""
        super().__init__()

    def lookup_term_or_id(self, term_or_id_or_name: str) -> Tuple[str, str, str, List[str]]:
        """
        Look up a term in PubChem by ID or name.

        Args:
            term_or_id_or_name: Part after 'PubChem:' prefix has been removed

        Returns:
            Tuple of (id, name, definition, synonyms)
        """
        ontology_prefix = 'PubChem'
        ontology_name_ols = 'pubchem'
        ...
        try:
            ...
            raise ValueError(f'Error preprocessing PubChem lookup input "{term_or_id_or_name}": {str(e)}') from e
        ...
        try:
            ...
            raise ValueError(f'PubChem lookup failed for {lookup_type_msg}: {str(e)}') from e
