"""
NDI Ontology RRID - Research Resource Identifiers.
...
class RRID(Ontology):
    ...
    def __init__(self):
        """Constructor for the RRID ontology object."""
        super().__init__()

    def lookup_term_or_id(self, term_or_id_or_name: str) -> Tuple[str, str, str, List[str]]:
        """
        Look up a term in RRID by ID or name.

        Args:
            term_or_id_or_name: Part after 'RRID:' prefix has been removed

        Returns:
            Tuple of (id, name, definition, synonyms)
        """
        ontology_prefix = 'RRID'
        ontology_name_ols = 'rrid'
        ...
        try:
            ...
            raise ValueError(f'Error preprocessing RRID lookup input "{term_or_id_or_name}": {str(e)}') from e
        ...
        try:
            ...
            raise ValueError(f'RRID lookup failed for {lookup_type_msg}: {str(e)}') from e
